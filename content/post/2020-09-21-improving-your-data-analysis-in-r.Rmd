...
Finally, we might like to have a better description than cbc or skm to ensure any collaborators understand exactly what we have.
...
samples <- tibble(
    ...
    dplyr::mutate(
        ...
        description = case_when(
            tissue == "cbc" ~ "Cerebellar Cortex",
            tissue == "skm" ~ "Skeletal Muscle"
        )
    )
```


Now we have our metadata, it might be a good idea to summarise this in the plain text parts of your R Markdown and present a table.
The package `pander` is very helpful for generating basic html tables.

::: {#add-pander .action}
Add `library(pander)` to the packages chunk and execute the chunk to make sure it's loaded into your interactive session.
:::
...
samples %>% 
    ...
    tally() %>%
    pander(caption = "Experimental groups and sample size")
```


Whilst this clearly isn't the full setup for an RNA-Seq experiment, these are pretty close to my first steps.
...
- Write summaries of your data as part of the R Markdown to ensure you have what you think you have, and to simply describe it accurately
